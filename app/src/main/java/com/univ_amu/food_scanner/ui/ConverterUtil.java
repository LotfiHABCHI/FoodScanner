...
public class ConverterUtil {
    ...
    private static String getStringFromDate(Resources resources, Date date) {
        ...
        if (dayCount == 0)
            return
                    resources.getString(R.string.today); //"Aujourd'hui" /* TODO */;
        if (yearCount >= 1)
            return resources.getQuantityString(R.plurals.years_ago,
                    yearCount, yearCount);
        /* TODO */;
        if (monthCount >= 1)
            return resources.getQuantityString(R.plurals.months_ago,
                    monthCount, monthCount);/* TODO */;
        return  resources.getQuantityString(R.plurals.days_ago,
                dayCount, dayCount);
